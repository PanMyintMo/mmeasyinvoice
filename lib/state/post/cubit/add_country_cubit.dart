...
class AddCountryCubit extends Cubit<AddCountryState> {
  ...
  Future<void> addWard(AddWardRequestModel addWardRequestModel) async {
    ...
    try {
      final data = addWardRequestModel.toJson();
     
      final response = await _authRepository.addWard(data);
     
      final wardResponse = AddWardResponse.fromJson(response);

     
      emit(AddWardSuccess(wardResponse));
    } catch (error) {
      emit(AddCountryFailed(error.toString()));
    }
  }
